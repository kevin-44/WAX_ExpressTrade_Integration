...
Create a `.php` file (or download the `blank.php` file in this repository), name it `index.php`, place it inside of your `public_html` directory, open the file with a text editor ([Sublime Text](https://www.sublimetext.com/3) is recommended) and include `execute_api_call.php` (you can download the file from this repository). Your `index.php` file should contain the following:
...
<?php
	...
	$response = ExecuteAPICall("GET", "IItem/GetItems/v1", array("key=apikey&sku_filter=100,102")); // method (POST or GET), endpoint, data
