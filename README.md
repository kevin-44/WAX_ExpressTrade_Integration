...
Some endpoints and [WAX ExpressTrade](https://trade.opskins.com) itself require 2FA (*Two Factor Authentication*) to be enabled on your account, you can do this with the help of your mobile device and the Google Authenticator app; however, it is recommended that you use any other app (this could also be a Chrome extension) that shows you your `secret` as it is also required in some cases - so that everything is automated.
